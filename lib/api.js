...
/**
 * Sign a message using the private key associated with an address
 * @private
 * @memberof Client.API
 * @param {String} message - The message to sign
 * @param {String} path - The relative path used to derive the address
 * @param {String} [password] - The password to decrypt the private key
 * @return {String} - The signature
 */
API.prototype.signMessage = function(message, path, password) {
  $.checkState(this.credentials);
  $.checkState(this.credentials.addressType == Constants.SCRIPT_TYPES.P2PKH, 'Address type must be P2PKH');

  var self = this;

  try {
    var basePk = self.credentials.getDerivedXPrivKey(password);
    var derivedPk = basePk.deriveChild(path);
    var signature = Message(message).sign(derivedPk);
    return signature;
  } catch (ex) {
